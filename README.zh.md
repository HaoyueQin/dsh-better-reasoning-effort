...
- **暂存后统一落盘（编辑期零写入）**：编辑块也会出现在"尚未保存"的行上，两种形态同一条链路——新建供应商的卡片（自动适配直接用卡上填写的协议/端点），以及给**已保存供应商新增的模型行**（自动适配改用文档里的路由事实 + 行上已输入的显示名称）。**应用**把声明留在内存中，等你**离开编辑卡片**（或离开模型设置页）时才写入文档——官方卡片打开期间冻结了自己的设置版本号，插件此刻写入会让你在该卡片里的下一次保存被 `settings/conflict` 拒绝（"配置好了、保存了、却又变回原样"的成因）；绝不覆盖文档里已有的声明。注意暂存阶段表达不了"故意留空"：全部清空后再暂存等同撤单、退回自动适配的填充——确要不声明的模型，请先保存行，再在行内全不勾并应用（写入持久撤销标记）。
...
2. 展开某个模型行：官方容量字段下方是编辑块。
   ...
   - 点「应用」：声明先留在内存（提示「已暂存」），**离开编辑卡片**（或离开模型设置页）时自动写入文档。

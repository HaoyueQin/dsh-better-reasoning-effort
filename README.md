...
- **Request headers and `user-agent` (issue #12, inside the provider card)**: every provider card gains a "Request headers" section editing exactly the official `llm-pi-ai.providers.<route>.headers` — the one channel DSH sends, which is provider-level and has no input anywhere in the UI. It rides the official keyed `settings.models.provider-card` extension seat (dispatched per adapter family namespace), so it belongs to the card's own editing flow without a single DOM anchor. It appears **only while the card is being edited, and never changes the list's height**: the provider list carries neither the row nor any placeholder for it. (The official slot mounts this section inside a `display: contents` container, so the plugin's own root IS a flex item of the card row — and that row spaces its children with a `gap: 12px`. A wrapper left in the flow while empty therefore adds a phantom 12px to EVERY card: measured live as a 54px → 66px card-height regression, now pinned by a test.) Pressing the card's own Edit — the gesture that opens the model list — reveals it alongside. That is deliberate: the list is for choosing a provider, not for configuring one. Within the section it is collapsed again (title + count + ›), and opening that discloses the hint, the entries and the Edit action; values render **masked**, and the write is a path-merge through `settings.mutate` that cannot touch the profile's `baseURL` / `models` / `compat`. Revealing the section reads the official card's edit state off the DOM: the official row exposes no data attribute and no `aria-expanded` for its editor (the open state lives in section-internal React state), so the mounted editor's CSS-module class is the only signal there is — if an official build renames that class root, the section stays hidden and the official page is unaffected (see known limitations).
- **The plugin takes `user-agent` over at the request layer**: the official adapter discards a configured `user-agent` — the name is reserved for client attribution, so `requestHeaders()` filters it out and merges the harness identity back in, meaning a `headers` entry alone never reaches the wire. Gateways that fingerprint the client identity (agentrouter and claude-code-router style relays) therefore never see the configured value, so the plugin performs that one override at the **fetch layer**. Scope is an exact **origin** match on the profile's `baseURL` (not a substring match: `http://host` cannot claim `http://host.evil`); the model-listing probe and official discovery share that origin, so they are covered too. Two routes declaring different User-Agents for one origin are **reported rather than guessed** at (the fetch seam cannot tell their requests apart); `uaOverride: false` stops the layer from matching anything, for handing the job to another plugin.
- **Coexistence detection**: three signals are read — whether the installed `dsh-llm-pi-ai` already carries a user-agent patch (a patched deployment can just fill in the `headers` above and needs no request-layer takeover), whether a known sibling plugin (`dsh-client-masquerade` / `dsh-llm-headers` / `dsh-llm-pi-ai-headers` / `dsh-custom-provider-settings`) sits in a reachable `node_modules`, and any same-origin disagreement. Anything found is surfaced on the card; nothing fights silently.
...
Compiled and gated against `0.1.7-alpha.1` (typecheck, the test suite and the full build all run against that release's official packages); the most recent **live-kernel** runtime baseline is `0.1.7-rc.2` (the request-header work was verified end-to-end on it — see the live re-check below), previously `0.1.5-rc.1`.
...
- **Credentials inside `headers` are not redacted**: the official README warns that the harness redactor does not reach values in `headers`. This plugin masks them in the read-only view (so a shared screen does not expose them), but the settings document — `settings.yaml`, or `profiles/<name>/cordis.patch.yml` on `0.1.7`+ — still holds them in clear text. Treat that file like an API key.
- **The request-header section's edit-state detection reads an unofficial signal**: the official provider row exposes no data attribute and no `aria-expanded` for its editor (the open state lives in `ModelsSection` component state), so the plugin infers it from the mounted editor's CSS-module class (`_editor*`). If an official build renames that class root, the section stops appearing — the failure mode is "the feature is gone", never "the page is broken"; the official page is unaffected, and the upgrade path is an official data attribute or an `editing` field on the slot occurrence.
- **Only one `user-agent` rewrite should be active**: `dsh-llm-headers`, `dsh-llm-pi-ai-headers`, `dsh-client-masquerade` (which patches the official package file) and this plugin's fetch layer land on the same layer, and the last writer on the wire wins. The plugin detects and reports this, but the detection covers known package names and whether the official bundle carries a patch.
- **An agentrouter-style gateway may also fingerprint the request body** (Claude Code's system blocks, tool names): a `user-agent` override alone can still draw a 429/503 that looks like queueing. That is beyond this plugin's scope.
- **The request-layer takeover relies on the official adapter creating a fresh SDK client per request**: measured on `0.1.7-rc.2` (pi-ai 0.85.1 + `@anthropic-ai/sdk` 0.123.0, whose `getDefaultFetch()` reads the global `fetch` dynamically), with an end-to-end test guarding that boundary. If a future release captures a `fetch` reference at construction time, this takeover stops working — and the test, rather than a silent failure, is what will say so.

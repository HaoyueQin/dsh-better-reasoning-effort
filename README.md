...
- **DOM injection**: `reconcile()` in `src/client/injection/models-page-editor.ts` locates model rows by the official button aria-label (`Capacities`/`容量`) and mounts the editor into the capacity disclosure. The browser half's assembly layer is `src/client/index.ts`; each injection seam lives in `src/client/injection/`.
